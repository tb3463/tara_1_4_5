...
def frame_all_images(directory=None):
    ...
    directory_1 = os.path.join(directory, 'modified1')
    try:
        os.mkdir(directory_1)
    except OSError:
        pass
    item_list = get_images(directory)
    for i in range(len(item_list)):
        mask = PIL.Image.new('RGBA',(width,height),(50,50,50,200))
        current_image = i
        edited = current_image.resize(100,100)
        edited_2 = (edited.paste(0,0),mask)
        edited_2_filename = os.path.join(directory_1)
        edited_2.save(edited_2_filename)